...
impl Element for TextElement {
    ...
    fn prepaint(
        ...
        let (display_text, text_color) = if is_empty {
            // Derive the placeholder from the element's own text color so it
            // stays legible when the input is drawn on a light surface.
            (&Rope::from(placeholder.as_str()), style.color.opacity(0.5))
    ...
    fn paint(
        ...
        if focused && show_cursor {
            if let Some(cursor_bounds) = prepaint.cursor_bounds_with_scroll() {
                // Match the text color so the caret stays visible on inputs
                // drawn over a light surface.
                window.paint_quad(fill(cursor_bounds, window.text_style().color));
            }
        }
